...
3. *Calibration*: a calibrated plugin divides distances by its estimated σ², so repeated observations of the same bits combine by summing (`combine_llrs_map`) — never by re-weighting with 1/σ², which would apply σ⁻² twice. `crates/openpulse-modem/tests/llr_calibration.rs` fails any plugin whose `mean(|LLR|)` stops growing with SNR — which is the *weak* bar. That a magnitude means what it says, within 4×, is checked only by the five `llr_reliability.rs` files; bpsk, qpsk and psk8 pass the weak bar and face no bin test.
