...
| The notch's DETECTION is independent of the caller's block size, its filter actually suppresses across a stream of small blocks, and a short block is windowed by a Hann of its own length (#1303) — the daemon delivers ~400-sample ticks, where analysing the raw block cost ~10 dB of processing gain and re-priming the biquads every block held suppression at **11.9 dB** instead of the design figure. Fast gates, because REQ-QRM-01's own suite is held out of the gate and so checks none of this per-PR | `cargo test -p openpulse-dsp --no-default-features --lib notch` |
...
| The daemon's carrier detect tracks the **band noise floor** instead of a fixed squelch (REQ-DCD-01) — recorded idle at 0.126 RMS is not a carrier, and a real frame in that floor still flushes one bounded burst. Both assertions are swept across block sizes **spanning** the 512-sample analysis window: the tracker is chunking-invariant (#1254), and the gate used to pass only because its fixture fed 800-sample blocks the daemon never produces. The negative control filters to bursts that could BE the frame — it was satisfiable by a 400-sample burst on pure idle, i.e. unable to fail, at every block size below the window | `cargo test -p openpulse-modem --no-default-features --test daemon_squelch_noise_floor` + `cargo test -p openpulse-dsp --no-default-features --lib noise_floor` |
| The correlation veto's residual-frequency grid step follows the template's **time span**, not its decimated sample count (#1062) — `0.25*fs/len` is `1/duration` only on the passband arm, so on the DDC arm the grid came out `decim`x too coarse and the worst-case coherent loss was `\|sinc(decim/8)\|`: 0.974 at 1, **0.637 at 4**, an exact **null** at 8. Latent only because `MODES_WITH_VETO` holds BPSK250 alone (passband); publishing any slow rung activates it. The equivalence bench could not see it (it compares the arms at a SINGLE frequency hypothesis) and neither could the membership pin (its checklist names the constants that travel WITH the template, not the one the engine derives) | `cargo test -p openpulse-modem --no-default-features --lib ddc_veto_arm::the_grid_step_follows_the_template_span_not_its_decimated_length` |
| The correlation veto's **stand-down is recorded on every acquisition path** (#1342). #1157 specified it as "latched with hysteresis, logged"; the daemon's phase-2 veto (#1168) decided stand-downs but never updated the announce latch, its `warn!`, or `rho_stand_down_settles`, so there it was neither reported nor logged. One helper, `decide_preamble_veto`, now owns the decide-and-record step for both paths. A **recording** gate, not evidence that a real station stands down: it primes the calibration (64 × ρ 0.40 → derived 0.72, above BPSK250's 0.50 bound). The CLI arm is the instrument control. Sabotage-verified both ways — the daemon site's pre-fix inline code fails both daemon arms at the latch while the CLI arm passes, and a helper that stops counting a stand-down as an accept fails all three at the counter | `cargo test -p openpulse-modem --no-default-features --lib stand_down_is_recorded_on_every_path` |
...
| A QSY line is authenticated to the **handshake** peer and carries a signed freshness bound — the daemon called the unsigned codec, so `verify_line` had zero callers outside its own crate while a dozen docs (one of them a code review) said the opposite (#1252) | `cargo test -p openpulse-daemon --no-default-features --test qsy_lines_are_authenticated` + `cargo test -p openpulse-qsy` |
| A repeater that PANICS is recovered, and does not come back holding the key — `join()` returns the payload rather than the value, so the object died with the thread and only a daemon restart brought it back. The session is wrapped in `catch_unwind` at SESSION level (a per-burst catch that continued would be a real hidden crash loop) and a panic is reported as a panic, distinct from #1328's designed `MAX_SENSE_FAULTS` stop. **The hardware half matters more than the recovery**: in full duplex the live `PttKeyGuard` lives in `session_guard`, not on the stack, and unwinding does not clear it — rig_b was released before only because the closure DROPPED the whole repeater, so keeping it alive would hand back a keyed transmitter bounded only by the silence watchdog. The existing `shared_ptt` unwind test is the artificially-easy fixture here (stack guard); this one holds the guard where the repeater really holds it | `cargo test -p openpulse-repeater --no-default-features --test abnormal_exit_release` + `cargo test -p openpulse-daemon --no-default-features --lib a_panicking_session_is_reported_as_a_panic_and_returns_the_repeater` |
| A stopped cross-band repeater can be started again (#1324) — the thread OWNS it, and a closure returning `()` dropped it the moment `run_full_duplex` returned, so `DisableRepeater` left "no repeater is available" and only a daemon restart recovered. For an unattended §97.221 station that means the control point can stop the transmitter but not restart it. The thread now returns it; both join sites (`DisableRepeater` and the error-exit reap) put it back. Asserted on `repeater_stop`, not on the event, because `RepeaterChanged { enabled: true }` was emitted on the broken build too. Also gated here: an error exit emits exactly ONE `RepeaterChanged { false }` (thread and reap each emitted one), and bursts queued before a session are **discarded rather than transmitted** — measured at 4 bursts / 4 keyings of audio as old as the pause, counted by a tripwire so a drain that never runs cannot pass | `cargo test -p openpulse-daemon --no-default-features --lib -- a_disabled_repeater_can_be_enabled_again an_error_exit_reports_once_and_still_returns_the_repeater` + `cargo test -p openpulse-repeater --no-default-features --test carrier_sense bursts_queued_before_a_session` |
| The cross-band repeater does not key rig_b while rig_b's band is busy (#1325) — it is the one automatic transmitter here that keys a rig the daemon never listens to, so a decode on the INPUT used to be sufficient reason to transmit on the OUTPUT. It senses through the same `InputCapture` seam and calibrated noise floor the receiver uses, on rig_b's own card. Three rules, each sabotage-verified to fail its OWN case and no other: a busy band defers the burst; a band that cannot be READ counts as busy, not clear; and a `full_duplex` session already holding the key does not sense against its own carrier. Carries the control that stops it passing vacuously — the identical burst and band DO relay with `carrier_sense = false` — and counts deferrals, because a test that only shows a relay on a clear band cannot tell sensing from a no-op. **`enable_csma()` is NOT the fix**: the DCD is fed only by the RX capture seam, so it reads permanently idle, and its p-persistence refusal ends the whole session on the first unlucky roll (measured) | `cargo test -p openpulse-repeater --no-default-features --test carrier_sense` |
| The RX burst cap covers the **relay consumer's** rung, not just this station's (#1308) — under that issue's decision the repeater reads the bursts the daemon flushes, so a cap sized from `[modem] mode` alone truncates exactly the frames the repeater exists to forward when its rung is slower. The #1249 class, one consumer over. Carries its own positive control: without the declaration the same frame IS truncated, so the gate cannot pass on a frame that merely fits | `cargo test -p openpulse-modem --no-default-features --test burst_cap_tracks_the_relay_rung` + `cargo test -p openpulse-daemon --no-default-features --lib enabling_the_repeater_widens_the_burst_cap_to_its_rung` |
| The daemon's **receive tick** drops its capture stream after transmitting (#1319) — that arm keys the OTA ACK, a CONACK or QSY reply, and the periodic §97.119 station ID, yet #1007's post-transmit rule was enforced on the command arm only. Not an edge case: every compliant station IDs on a schedule. Gated through a real daemon on a counting backend; one command produces two keyed transmits on two arms, so the reopen count discriminates (2 with the fix, 1 without) — and the ID timer must be ARMED by a prior transmit or the fixture is vacuous | `cargo test -p openpulse-daemon --no-default-features --test ota_ack_capture_stream` |
| The QSY scan opens **no capture stream of its own** (#1312) — it ran `engine.receive` per candidate while the daemon's own `rx_stream` was held, two concurrent captures on one device (#1007's rule), and the receive was inert anyway: `last_rx_snr_db` only moves after a frame validates, so every candidate scored the same stale number from the home frequency. Asserted at the BACKEND with a counting backend; the pre-fix count is 3, one per candidate, which is what proves the instrument is wired | `cargo test -p openpulse-daemon --no-default-features --lib the_qsy_scan_opens_no_capture_stream_of_its_own` |
...
| The FSK4-ACK scan reaches the **whole listen window**, and a foreign-session ACK cannot starve it (#1247) — the onset scan was capped at 16 000 samples (~2 s) inside a window sized at 4–9 s, and it restarted from zero every tick, so a co-channel ACK ahead of ours won the scan deterministically for the whole window. Both fixtures deliver the capture in CHUNKS and are deliberately misaligned from the chunk grid: an ACK that lands on a boundary is decoded by the per-chunk whole-buffer path and the scan is never exercised — measured, an aligned fixture passes with the scan fully sabotaged | `cargo test -p openpulse-modem --no-default-features --test fsk4_ack_scan_reaches_the_whole_window` + `--lib ack_scan_tests` |
